...
import {
  orderUserShopAllAPI,
  orderUserShopCidAPI, orderUserShopGeAPI, orderUserShopLeAPI, orderUserShopOidAPI, orderUserShopOsateAPI,
...
export default {
  ...
  methods:{
    chooseValue(label) {
      ...
      }else if (this.choose.includes('用户编号')) {
        this.$message({
          message: '请输入用户编号，查询相应订单',
          type: 'success'
        });
      ...
      if (this.tag === 0) {
        ...
        if (this.choose.includes('小于')) {
          ...
          if (numRe.test(this.value)) {
            console.log("拿到输入值-数字", this.value)
            console.log(this.value)
            this.aPage.cid = Number(this.value)
            console.log("this.aPage.oid"+this.aPage.oid)
            this.getOrderICidList()
          }else{
            console.log("非数字", this.value)
            console.log(typeof this.value)
            this.$message({
          ...
          if (this.value.includes('已支付')){
            this.aPage.osate=1
            console.log(typeof this.aPage.osate)
    ...
    async getOrderLeList() {
      ...
      const {data: res} = await orderUserShopLeAPI(json);
      console.log("开始");
      if (res.code === '00000') {
        this.orders=[]
    ...
    async getOrderGeList(){
      ...
      const json = JSON.stringify(this.aPage);
      const {data: res} = await orderUserShopGeAPI(json);
      console.log("开始");
      if (res.code === '00000') {
        this.orders=[]
    ...
    async getOrderICidList(){
      console.log("getOrderICidList")
      const json = JSON.stringify(this.aPage);
      const {data: res} = await orderUserShopCidAPI(json);
      console.log("开始");
      if (res.code === '00000') {
        this.orders=[]
        console.log("通过用户编号获取成功");
        console.log(res);
        this.orders = res.data.records;
        this.aPage.total = res.data.total;
        this.$message({
    ...
    async getOrderOidList(){
      ...
      const json = JSON.stringify(this.aPage);
      const {data: res} = await orderUserShopOidAPI(json);
      console.log("开始");
      if (res.code === '00000') {
        this.orders=[]
    ...
    async getOrderOsateList(){
      ...
      console.log(this.aPage)
      const json = JSON.stringify(this.aPage);
      const {data: res} = await orderUserShopOsateAPI(json);
      console.log("开始");
      if (res.code === '00000') {
        this.orders=[]
    ...
    async getList() {
      ...
      console.log("this.aPage.uid")
      console.log(this.aPage.uid)
      const json = JSON.stringify(this.aPage);
      const {data: res} = await orderUserShopAllAPI(json);
      console.log(res)
      if (res.code === '00000') {
        this.orders = res.data.records;
        this.aPage.total = res.data.total;
  ...
  created() {
    ...
    this.aPage.uid = JSON.parse(localStorage.getItem('user')).uid
    console.log("created")
    console.log(this.aPage.uid)
    this.getList()
  },
